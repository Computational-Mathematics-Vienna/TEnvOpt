...
WORKpath = pwd;
if ~strcmp(extractAfter(WORKpath,length(WORKpath)-10),"TEnvOpt")
  WORKpath = input(['Please enter the directory of TEnvOpt ',...
  '(e.g. ~/Software/TEnvOpt):'])
end
...
if nomad
  ...
  if s ~= 0
    error('tar failed. Please extract TEnvOpt/SOLVERS/v.4.4.0.tar.gz manually')
  end
...
disp('Test starts:')
TESTmin
disp('Please change the usersName (path to TEnvOpt) in /TE/TESTall (line 20),')
